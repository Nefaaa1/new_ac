...
- **Liste** : tableau coloré + `WithSorting` (seul `nom` triable) + recherche libre `#[Url(except:'')] $search` (nom / société / nom-prénom client), `accessibleBy`. Colonnes : **Site** (nom + chip statut dessous, et à droite picto paiement agence — avec `paiement_mois` en petit dessous s'il est rempli — + picto boutique), **Client**, puis **Hébergement / FTP / BDD / WordPress** = `<x-admin.fill-indicator :filled="$site->relation?->hasData()">` (check emerald = renseigné, tiret zinc = vide). Les 4 relations credentials sont eager-loaded.
