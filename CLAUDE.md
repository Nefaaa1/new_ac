...
## Tickets (CRUD)
- `Admin\Tickets` (route `admin.tickets`) : CRUD en **slide-over** (`max-w-xl`, UX clients/actions) — table colorée + `WithSorting` (défaut `date desc`, triable `date`/`demande`) + recherche libre `#[Url(except:'')] $search` (demande / descriptif / nom du site) + soft delete.
...
- **Liste** (colonnes) : **Site** (nom + société, badge archivé) · **Date** · **Demande** (pastille couleur = importance + descriptif tronqué) · **Statut** (chip couleur) · **Attribué à** (avatar + nom) · **Temps** (si défini) · **Devis** (chip si `a_deviser`) · Actions.
- **3 filtres** (`#[Url(except:'')]`) : `utilisateurFilter` (`'none'` = non attribués, sinon id admin — **admins uniquement**, `adminsList()` = admins non suspendus), `statutFilter` (statut_id), `devisFilter` (`'sans'` = pas de devis, sinon id `devis_statut` avec `a_deviser=true`).
