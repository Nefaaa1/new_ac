<div>
    <x-admin.page-header
        title="Statuts"
        subtitle="Définir les statuts applicables aux sites (libellé, couleur, date requise)."
        icon="tags" />
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-primary text-left text-xs font-semibold uppercase tracking-wider text-white">
                    ...
                    <x-admin.sort-header field="requiert_date" label="Date requise" :sort="$sortField" :direction="$sortDirection" />
                    <x-admin.sort-header field="sites" label="Sites" :sort="$sortField" :direction="$sortDirection" />
                    <th class="px-5 py-3.5 text-right">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-zinc-100">
                @forelse($this->statuts as $statut)
                    <tr wire:key="statut-{{ $statut->id }}" class="transition odd:bg-white even:bg-primary/[0.04] hover:bg-secondary/10">
                        <td class="px-5 py-3">
                        ...
                        <td class="px-5 py-3">
                            @if($statut->requiert_date)
                                <span class="inline-flex items-center gap-1 rounded-full bg-secondary/15 px-2.5 py-0.5 text-xs font-medium text-secondary">
                                    <x-lucide-calendar-check class="h-3.5 w-3.5" /> Oui
                                </span>
                            @else
                                <span class="text-zinc-300">—</span>
                            @endif
                        </td>
                        <td class="px-5 py-3 text-zinc-600">{{ $statut->sites_count }}</td>
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button wire:click="deleteStatut({{ $statut->id }})" type="button" title="Supprimer"
                                        wire:confirm="Supprimer ce statut ? Les sites qui le portent n'auront plus de statut affiché."
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-5 py-10 text-center text-sm text-zinc-400">Aucun statut pour le moment.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div x-show="open"
            ...
            <form wire:submit="save" class="flex flex-1 flex-col overflow-hidden">
                <div class="flex-1 space-y-6 overflow-y-auto px-6 py-6">
                    ...
                    {{-- Date requise --}}
                    <x-checkbox wire:model="requiert_date"
                                label="Date de statut obligatoire"
                                hint="Si coché, un site portant ce statut devra renseigner sa date." />
                </div>
