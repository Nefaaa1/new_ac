<div>
    <x-admin.page-header
        title="Statuts devis"
        subtitle="Définir les états de devis des tickets (libellé, couleur, ordre)."
        icon="file-check" />
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-primary text-left text-xs font-semibold uppercase tracking-wider text-white">
                    ...
                    <x-admin.sort-header field="libelle" label="État" :sort="$sortField" :direction="$sortDirection" />
            ...
            <tbody class="divide-y divide-zinc-100">
                @forelse($this->statuts as $statut)
                    <tr wire:key="devis-statut-{{ $statut->id }}" class="transition odd:bg-white even:bg-primary/[0.04] hover:bg-secondary/10">
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button wire:click="deleteStatut({{ $statut->id }})" type="button" title="Supprimer"
                                        wire:confirm="Supprimer cet état ? Les tickets qui le portent n'auront plus d'état de devis affiché."
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-5 py-10 text-center text-sm text-zinc-400">Aucun état pour le moment.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div x-show="open"
            ...
            <div class="flex shrink-0 items-center justify-between border-b border-zinc-100 px-6 py-4">
                <h2 class="text-base font-semibold text-zinc-900">
                    {{ $editingId ? "Modifier l'état" : 'Nouvel état' }}
                </h2>
            ...
            <form wire:submit="save" class="flex flex-1 flex-col overflow-hidden">
                <div class="flex-1 space-y-6 overflow-y-auto px-6 py-6">
                    <x-text-input label="Libellé" name="libelle" required floatError wire:model="libelle"
                                  placeholder="Ex. À deviser, Validé…" />
